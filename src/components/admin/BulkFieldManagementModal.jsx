...
import { MdClose, MdAdd, MdDelete, MdWarning } from "react-icons/md";

const BulkFieldManagementModal = ({
  isOpen,
  onClose,
  parentCategory,
  onFieldsUpdated,
  leafCategoriesCount = 0,
}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const confirmed = window.confirm(
        `Are you sure you want to add these fields to all ${leafCategoriesCount} leaf categories under "${parentCategory?.name}"? This will replace existing fields in all leaf categories.`
      );
      ...
      await onFieldsUpdated(validFields);
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-sm sm:max-w-4xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900">
              Add Form Fields to All Leaf Categories
            </h2>
            <p className="text-sm text-gray-600">
              Add fields to all leaf categories under{" "}
            ...
            {leafCategoriesCount > 0 && (
              <div className="flex items-center space-x-2 mt-2 text-sm text-orange-600">
                ...
                <span>
                  This will affect {leafCategoriesCount} leaf categories
                </span>
              </div>
            )}
          </div>
        ...
        <div className="p-4 sm:p-6 bg-orange-50 border-b border-orange-200">
          <div className="flex items-start space-x-3">
            <MdWarning className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0" />
            <div className="text-sm">
              <p className="font-medium text-orange-800">Important Notice</p>
              <p className="text-orange-700 mt-1">
                These fields will be added to{" "}
                <strong>all {leafCategoriesCount}</strong> leaf categories that
                are descendants of "{parentCategory?.name}". This will replace
                any existing fields in those categories.
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-white">
            ...
            >
              {isSubmitting
                ? "Applying to All Leaf Categories..."
                : `Apply to ${leafCategoriesCount} Leaf Categories`}
            </button>
          </div>
        </form>
      </div>
...
export default BulkFieldManagementModal;
